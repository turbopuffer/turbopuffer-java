...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { TurbopufferInvalidDataException("Value is not a string") }
                ...
                Optional.ofNullable(
                    (value as? List<*>)?.map {
                        try {
                            ...
                            // The known value is a list, but not all values are convertible to `JsonValue`.
                        ...
                        ?.map { (key, value) ->
                            ...
                            val jsonValue =
                                try {
                                    ...
                                    // The known value is a map, but not all items are convertible to `JsonValue`.
                                    return Optional.empty()
                                }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw TurbopufferInvalidDataException("`$name` is not set")
            is JsonNull -> throw TurbopufferInvalidDataException("`$name` is null")
            else -> throw TurbopufferInvalidDataException("`$name` is invalid, received $this")
        }
    ...
    @JvmSynthetic
    fun accept(consume: (T) -> Unit) = asKnown().ifPresent(consume)
