...
private constructor(
    ...
    companion object {
        ...
        /**
         * Returns a mutable builder for constructing an instance of [WriteResult].
         *
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(writeResult: WriteResult) = apply {
            billing = writeResult.billing
            message = writeResult.message
            rowsAffected = writeResult.rowsAffected
            status = writeResult.status
            deletedIds = writeResult.deletedIds.map { it.toMutableList() }
            patchedIds = writeResult.patchedIds.map { it.toMutableList() }
            performance = writeResult.performance
            rowsDeleted = writeResult.rowsDeleted
            rowsPatched = writeResult.rowsPatched
            rowsRemaining = writeResult.rowsRemaining
            rowsUpserted = writeResult.rowsUpserted
            upsertedIds = writeResult.upsertedIds.map { it.toMutableList() }
            additionalProperties = writeResult.additionalProperties.toMutableMap()
        }
        ...
        /** Alias for calling [addDeletedId] with `Id.from(string)`. */
        fun addDeletedId(string: String) = addDeletedId(Id.from(string))

        /** Alias for calling [addDeletedId] with `Id.from(integer)`. */
        fun addDeletedId(integer: Long) = addDeletedId(Id.from(integer))
        ...
        /** Alias for calling [addPatchedId] with `Id.from(string)`. */
        fun addPatchedId(string: String) = addPatchedId(Id.from(string))

        /** Alias for calling [addPatchedId] with `Id.from(integer)`. */
        fun addPatchedId(integer: Long) = addPatchedId(Id.from(integer))
        ...
        /** Alias for calling [addUpsertedId] with `Id.from(string)`. */
        fun addUpsertedId(string: String) = addUpsertedId(Id.from(string))

        /** Alias for calling [addUpsertedId] with `Id.from(integer)`. */
        fun addUpsertedId(integer: Long) = addUpsertedId(Id.from(integer))
        ...
        /**
         * Returns an immutable instance of [WriteResult].
         *
     ...
     *   expected type.
     */
    fun validate(): WriteResult = apply {
        if (validated) {
            return@apply
        }
    ...
    internal fun validity(): Int =
        (billing.asKnown().getOrNull()?.validity() ?: 0) +
            ...
            (deletedIds.asKnown().getOrNull()?.size ?: 0) +
            (patchedIds.asKnown().getOrNull()?.size ?: 0) +
            ...
            (upsertedIds.asKnown().getOrNull()?.size ?: 0)
