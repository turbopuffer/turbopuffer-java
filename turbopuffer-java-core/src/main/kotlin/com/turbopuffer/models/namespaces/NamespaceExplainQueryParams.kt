...
private constructor(
    ...
    /**
     * Number of documents to skip before returning results. Supported only in v2 queries with an
     * explicit `rank_by` and `top_k` or `limit`.
     *
     * @throws TurbopufferInvalidDataException if the JSON field has an unexpected type (e.g. if the
     *   server responded with an unexpected value).
     */
     ...
     * This arbitrary value can be deserialized into a custom type using the `convert` method:
     * ```java
     * MyClass myObject = namespaceExplainQueryParams.rankBy().convert(MyClass.class);
     * ```
     */
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is NamespaceExplainQueryParams &&
